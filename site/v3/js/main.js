...
(async function () {
  ...
  const res = await fetch('/v3/data/content.json');
...
function renderWorkGrid(work, directors) {
  ...
  // Active director banner (hidden by default)
  const directorBanner = document.createElement('div');
  directorBanner.className = 'work-director-banner';
  directorBanner.id = 'directorBanner';
  directorBanner.innerHTML = `
    <div class="work-director-banner__info">
      <img class="work-director-banner__photo" id="bannerPhoto" src="" alt="">
      <div class="work-director-banner__text">
        <span class="work-director-banner__name" id="bannerName"></span>
        <span class="work-director-banner__role" id="bannerRole"></span>
      </div>
    </div>
    <button class="work-director-banner__close" id="bannerClose">View All</button>
  `;
  section.insertBefore(directorBanner, grid);

  directors.forEach((d, i) => {
    const card = document.createElement('div');
    card.className = 'work-director-card';
    card.dataset.director = d.name;
  ...
  initDirectorFilter(work, directors);
}
...
// --- Director Filter ---
function initDirectorFilter(work, directors) {
  ...
  const viewMore = document.querySelector('.work-view-more');

  function getAllItems() {
    return [...grid.querySelectorAll('.work-grid__item'), ...moreGrid.querySelectorAll('.work-grid__item')];
  }

  // Click a director card to filter
  directorsRow.addEventListener('click', (e) => {
    const card = e.target.closest('.work-director-card');
    if (!card) return;

    const directorName = card.dataset.director;
    const idx = parseInt(card.dataset.index);
    const director = directors[idx];

    // If clicking the already-active director, deactivate
    if (_activeDirector === directorName) {
      clearDirectorFilter();
      return;
    }

    _activeDirector = directorName;

    // Update banner
    document.getElementById('bannerPhoto').src = director.photo;
    document.getElementById('bannerName').textContent = director.name;
    document.getElementById('bannerRole').textContent = director.role;
    banner.classList.add('is-active');

    // Highlight active card
    directorsRow.querySelectorAll('.work-director-card').forEach(c => c.classList.remove('is-active'));
    card.classList.add('is-active');

    // Show both grids, hide view more and directors row
    moreGrid.style.display = '';
    viewMore.style.display = 'none';
    directorsRow.style.display = 'none';

    // Filter all items across both grids
    const allItems = getAllItems();
    allItems.forEach(item => {
      if (item.dataset.director === directorName) {
        item.style.display = '';
        item.style.opacity = '0';
  ...
  // Clear filter
  bannerClose.addEventListener('click', clearDirectorFilter);
  ...
  function clearDirectorFilter() {
    ...
    // Hide more grid again, show view more and directors
    moreGrid.style.display = 'none';
    viewMore.style.display = '';
    directorsRow.style.display = '';

    // Reset all items in both grids
    const allItems = getAllItems();
    // Show only initial grid items, hide more grid items
    grid.querySelectorAll('.work-grid__item').forEach(item => {
      item.style.display = '';
      item.style.opacity = '0';
      item.style.transform = 'translateY(20px)';
    });
    moreGrid.querySelectorAll('.work-grid__item').forEach(item => {
      item.style.display = '';
    });
  ...
  // Also handle nav "Directors" link to scroll to director cards
...
function initWorkFilters() {
  ...
  if (directorsRow) {
    ...
    window.addEventListener('scroll', () => {
      if (_userClicked || _activeDirector) return;
