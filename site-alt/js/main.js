...
function initHeroControls() {
  ...
  const heroProgressFilled = document.getElementById('heroProgressFilled');
  ...
  video.addEventListener('timeupdate', () => {
    if (video.duration) {
      ...
      heroProgressFilled.style.width = pct + '%';
      heroTimecode.textContent = formatTime(video.currentTime) + ' / ' + formatTime(video.duration);
    }
  });
...
function renderWorkGrid(work) {
  ...
  const filterNames = ['All', 'Post-Production', 'Motion Graphics'];
  ...
  work.forEach((project, i) => {
    ...
    item.className = `work-grid__item work-grid__item--${project.size}`;
  ...
  player.innerHTML = `
    <div class="work-player__close-outer">
...
function initWorkInteractions(work) {
  ...
  const closeOuter = player.querySelector('.work-player__close-outer');
  const closeBtn = closeOuter || player.querySelector('.work-player__close');
  let infoTimer;
